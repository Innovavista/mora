...
function koPantalla(json, opciones) {
    ...
    self.getSaldoAnulacionCarga = function () {
        ...
        let total         = 0;
        const notaCredito = self.notaCredito();
        const lineas      = notaCredito.lineas();
        for (let i = 0; i < lineas.length; i++) {
            const linea = lineas[i];
            const tipo  = typeof linea.tipo === "function" ? linea.tipo() : '';
        ...
        const saldo = comensal.saldoFloat();
        return saldo - total;
    };
    ...
    self.comprobarAlertarSaldoNegativo = ko.computed(function () {
        ...
        for (let i = 0; i < lineas.length; i++) {
            const linea      = lineas[i];
            const tipo       = typeof linea.tipo === "function" ? linea.tipo() : '';
            const reembolsar = linea.cantidadAReembolsar();
